...
def naked_twins(values):
    ...
    for unit in unitlist:
        ...
        new_dict = dict([(box, value) for box, value in unit_dict.items() if len(value) == 2])

        if len(new_dict) >= 2:
            twin_candidate = [value for value in new_dict.values()]
            twin_values = set([value for value in twin_candidate if twin_candidate.count(value) >= 2])
            twins = {box: value for (box, value) in new_dict.items() if value in twin_values}
